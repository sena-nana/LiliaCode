...
| history search | 搜索可导入 provider 历史。 | search term、cursor、limit、archived。 | 返回 provider history summary 列表。 | provider 无原生 search 时使用 Lilia 本地解析或空结果。 |
| history preview | 预览 provider 历史并映射为 Lilia timeline。 | provider session / thread id、detail。 | 返回 messages、events、event count。 | 解析失败时返回错误，不创建 task。 |
| history attach | 将 provider 历史绑定到 Lilia task。 | provider session / thread id、mode、task / project id。 | 创建或更新 task、写 attach event、后台同步 timeline。 | attach 失败不得写半成品 session checkpoint。 |
...
| `PluginsOverview` | 插件页统一概览。 | project cwd。 | 返回 Claude / Codex 扩展列表、配置路径、warnings。 | provider 专属字段无内容时返回空集合。 |
| `CodexMcpServer` / `CodexMcpServerInput` | 管理 Codex MCP server 配置。 | name、command、args、env、enabled。 | 更新 Codex config 并进入 runtime extensions。 | Claude adapter 不消费该配置。 |
| `ClaudeMcpServer` / `ClaudeMcpServerInput` | 管理 Claude MCP server 配置。 | name、command、args、env、enabled。 | 更新 Claude MCP config 并进入 runtime extensions。 | Codex adapter 不消费该配置。 |
| `ClaudeSkill` | 管理 Claude Skills。 | scope、name、description、enabled、path。 | 启用后进入 Claude runtime extensions。 | Codex adapter 返回空集合。 |
| `ClaudePlugin` | 管理 Claude local Plugins。 | scope、name、version、enabled、path。 | 启用后进入 Claude runtime extensions。 | Codex adapter 返回空集合。 |
...
| Lilia 落点 | Lilia 接口含义 | Codex 原生语义与映射 | Claude 原生语义与映射 | 状态 |
|---|---|---|---|---|
| history search | 搜索可导入的 provider 历史，并返回 Lilia 可展示摘要。 | 原生语义：Codex `thread/search` 按关键词、分页、排序、sourceKinds、archived 搜索 threads；`thread/list` 可列出 threads。映射：`codex_thread_search` 有关键词用 search，无关键词可 list，结果归一为 Lilia history summary。 | 原生语义：Claude SDK session API 当前未接入；Claude Code 本地历史存储在 `~/.claude/projects/*.jsonl`。映射：`claude_session_search` 解析本地 JSONL 并归一为 Lilia history summary。 | 已接入 |
| history preview | 预览 provider 历史，映射为 Lilia messages / timeline event 计数。 | 原生语义：Codex `thread/turns/list` 分页读取 turns，可带 item detail；`thread/turns/items/list` 补拉某 turn items。映射：`codex_thread_preview` 先读 turns，缺失或截断时补拉 items，再映射 Lilia timeline。 | 原生语义：Claude 本地 JSONL 记录 session messages、tool events、results 等。映射：`claude_session_preview` 解析 JSONL message / event 并映射 Lilia timeline。 | 已接入 |
| history attach | 将 provider 历史绑定到 Lilia task，创建 checkpoint 并后台同步。 | 原生语义：Codex thread id 可作为后续 resume checkpoint；turns 可回补 timeline。映射：`codex_thread_attach` 把 thread 绑定到当前或新 Lilia task，写 attach event 并后台同步。 | 原生语义：Claude session id 可作为 `options.resume` checkpoint；本地 JSONL 可回补 timeline。映射：`claude_session_attach` 把 session 绑定到 Lilia task，写 attach event 并后台同步。 | 已接入 |
...
| provider session management | 预设完整 provider session 管理能力的当前子集。 | 原生语义：Codex threads 支持 search、list、turns、fork、archive、name set；remote / realtime 管理另属其他能力。映射：当前只通过 Lilia history、fork、title sync、archive sync 子集接入，完整管理落到 `lilia_session_management`。 | 原生语义：Claude SDK 提供 `listSessions()`、`getSessionMessages()`、`getSessionInfo()`、`renameSession()`、`tagSession()`、`deleteSession()` 等 session API。映射：这些 session 管理方法的 Lilia 落点是 `lilia_session_management`；当前使用本地 JSONL 与 Lilia task 元数据替代。 | 部分接入 |
...
| `PluginsOverview` | 插件页统一展示 Claude / Codex 扩展状态、配置路径和 warnings。 | 原生语义：Codex 当前可管理 MCP server 配置文件。映射：overview 返回 `codexMcpServers`、`codexConfigPath`、warnings；Claude 专属集合为空。 | 原生语义：Claude 可发现 Skills、local Plugins 和 MCP config。映射：overview 返回 `claudeUserSkills`、`claudeProjectSkills`、`claudeUserPlugins`、`claudeMcpServers`、`claudeMcpConfigPath`、warnings；Codex 专属集合为空。 | 已接入 |
| `CodexMcpServer` / `CodexMcpServerInput` | 管理 Codex MCP server 配置，并作为 Codex runtime extension 输入。 | 原生语义：Codex MCP server 配置定义 server name、command、args、env、enabled 等，并由 Codex runtime 加载。映射：Lilia 通过 `plugins_list_codex_mcp_servers`、create / update / delete / enabled / open config 命令维护配置。 | 原生语义：Claude 不消费 Codex MCP config。映射：Claude adapter 对该 Lilia 落点返回空集合 / 不注入，不能把 Codex MCP 配置混入 Claude。 | 已接入 |
| `ClaudeMcpServer` / `ClaudeMcpServerInput` | 管理 Claude MCP server 配置，并作为 Claude runtime extension 输入。 | 原生语义：Codex 不消费 Claude MCP config。映射：Codex adapter 对该 Lilia 落点返回空集合 / 不注入。 | 原生语义：Claude `options.mcpServers` 注册外部 MCP server，当前支持 stdio config。映射：Lilia 通过 `plugins_list_claude_mcp_servers`、create / update / delete / enabled / open config 维护配置，运行时进入 `options.mcpServers`。 | 已接入 |
| `ClaudeSkill` | 管理 Claude Skills，并在启用后传给 Claude SDK。 | 原生语义：Codex 没有 Claude Skills。映射：Codex overview 返回空集合，不消费。 | 原生语义：Claude `options.skills` 指定启用的 user / project skill。映射：Lilia 通过 list / create / delete / enabled 管理 skill，运行时写入 `options.skills`。 | 已接入 |
| `ClaudePlugin` | 管理 Claude local Plugins，并在启用后传给 Claude SDK。 | 原生语义：Codex 没有 Claude Plugins。映射：Codex overview 返回空集合，不消费。 | 原生语义：Claude `options.plugins` 接收 plugin config，例如本地 path；当前 Lilia 只支持用户级 local plugin 启停。映射：Lilia 通过 list / enabled 管理，运行时写入 `options.plugins`。 | 部分接入 |
...
| `lilia_provider_settings` | 高级 provider 设置统一入口：tool allow / deny、sandbox、additional directories、structured output、budget、raw events、environment fields 等。 | 原生语义：Codex thread / turn / settings 字段可承接 environments、runtime roots、permissions、experimental raw events、persistExtendedHistory、reasoning effort 等。映射：已接入字段继续通过现有 Lilia composer / profile；未接入字段进入该落点并默认 unsupported diagnostic。 | 原生语义：Claude SDK `allowedTools` / `disallowedTools` / `tools` 控制工具集合，`settings` / `managedSettings` / `settingSources` 控制设置来源，`sandbox` / `additionalDirectories` 控制执行边界，`outputFormat` 控制结构化输出，`maxTurns` / `maxBudgetUsd` 控制预算，`abortController` 控制中断。映射：这些字段的 Lilia 落点是 `lilia_provider_settings`；当前未接入字段返回 unsupported diagnostic。 |
...
| `lilia_session_management` | provider session 全管理：list、messages、info、rename、tag、delete、archive，且不得取代 Lilia task 作为应用层权威状态。 | 原生语义：Codex threads 支持 `thread/search`、`thread/list`、`thread/turns/list`、`thread/turns/items/list`、`thread/fork`、`thread/archive`、`thread/name/set`。映射：当前 search / preview / attach / fork / title sync / archive sync 是已接入子集；完整管理未接入时返回 unsupported diagnostic。 | 原生语义：Claude `listSessions()` 列 session，`getSessionMessages()` 读消息，`getSessionInfo()` 读元信息，`renameSession()` / `tagSession()` / `deleteSession()` 管理 session。映射：这些方法的 Lilia 落点是 `lilia_session_management`；当前使用本地 JSONL 与 Lilia task 元数据替代。 |
...
| `lilia_provider_settings` | provider 设置来源、sandbox、结构化输出、预算和中断策略的统一设置入口。 | 原生语义：Codex `thread/start` / `turn/start` 的 `environments` 可绑定外部执行环境，`experimentalRawEvents` 可要求原始事件，`responsesapiClientMetadata` 传客户端元数据；`command/exec.permissionProfile` 控制 Lilia 执行编辑后命令的权限 profile。映射：已接入 metadata 和 command exec profile；environments / raw events 等必须先进入 `lilia_provider_settings` 或更具体预设落点。 | 原生语义：Claude `settings` 注入 flag settings，`managedSettings` 注入策略层设置，`settingSources` 控制 user / project / local 设置来源，`sandbox` 配置执行沙箱，`additionalDirectories` 扩展可访问目录，`outputFormat` 启用结构化输出，`maxTurns` / `maxBudgetUsd` 设置执行和费用上限，`abortController` 从 SDK 层中断 query。映射：当前不暴露这些字段；未来统一从 `lilia_provider_settings` 进入。 | 部分接入 |
| `lilia_session_management` | session 恢复、从某点恢复、继续最近 session 和 transcript 存储都必须由 Lilia task checkpoint 约束。 | 原生语义：Codex `thread/resume.excludeTurns` 可恢复时排除 turns，`thread/fork.excludeTurns` 控制 fork 是否排除 turns，`thread/resume.path` / `thread/fork.path` 可按路径恢复 / fork。映射：Lilia 只使用 checkpoint thread id 和默认 `excludeTurns` 策略；path 不作为应用层输入。 | 原生语义：Claude `options.resume` 恢复指定 session，`options.continue` 继续最近 session，`options.resumeSessionAt` 从指定 message UUID 恢复，`sessionId` 可指定新 session id；`sessionStore` / `sessionStoreFlush` / `loadTimeoutMs` 控制 transcript mirror 到外部 store 的加载和刷盘。映射：已接入 `resume`；continue / resumeSessionAt / sessionId 落到 `lilia_session_management`，sessionStore 相关 alpha 能力落到 `lilia_experimental_capability`。 | 部分接入 |
...
| `AgentTimelineEvent` | hook、subagent、thinking 和系统状态事件只能作为 Lilia timeline / diagnostics 展示，不能绕过 Lilia 状态机。 | 原生语义：Codex plan / todo / command / file item 都是 app-server 事件；raw events 未产品化。映射：已归一为 timeline，未知事件走 tool / diagnostic 兜底。 | 原生语义：Claude `includeHookEvents` 控制是否输出全量 hook lifecycle，`forwardSubagentText` 转发 nested subagent 文本，`agentProgressSummaries` 生成子 agent 周期性摘要；`listSubagents()` / `getSubagentMessages()` 读取 subagent 列表和 transcript。映射：当前只镜像已收到的 hook / subagent / thinking 事件；完整 transcript 落到 `lilia_session_management`，进度摘要和事件开关落到 `lilia_provider_settings`。 | 部分接入 |
