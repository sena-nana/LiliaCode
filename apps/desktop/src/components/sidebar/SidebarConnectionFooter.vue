  ...
  quotaInflight = (async () => {
    try {
      ...
      if (seq === quotaRequestSeq) {
        officialQuota.value = result;
      }
    } catch (err) {
      if (seq === quotaRequestSeq) {
        officialQuota.value = quotaUnavailableStatus(err);
      }
    } finally {
...
<template>
  <div class="sb-footer">
    ...
    <RouterLink
      ...
      :aria-describedby="quotaTooltipOpen ? 'sb-conn-quota-popover' : undefined"
