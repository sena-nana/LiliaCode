...
describe("ChatComposer", () => {
  ...
  it("Codex 后端可从工具栏诊断 provider runtime 设置", async () => {
    const view = render(ChatComposer, {
      props: {
        state: codexState,
        attachments: [],
    ...
    await fireEvent.click(view.getByRole("button", { name: "Provider runtime 设置" }));
    await fireEvent.click(view.getByRole("menuitem", { name: "诊断当前设置" }));
  ...
  it("Codex 后端可从工具栏更新 provider runtime 设置", async () => {
    const promptSpy = vi.spyOn(window, "prompt");
    promptSpy
      .mockReturnValueOnce("gpt-5.5-codex")
      .mockReturnValueOnce("ask")
    ...
    await fireEvent.click(view.getByRole("button", { name: "Provider runtime 设置" }));
    await fireEvent.click(view.getByRole("menuitem", { name: /更新 runtime 设置/ }));
  ...
  it("Claude 后端可从工具栏更新 max turns", async () => {
    const promptSpy = vi.spyOn(window, "prompt");
    promptSpy
      .mockReturnValueOnce("claude-opus-4-5")
