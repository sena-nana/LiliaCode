...
//! and enforce Bearer auth on observe routes. `POST /agent/wire` always
//! requires Bearer (even on loopback). `/health` stays open.
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !local.ip().is_loopback() {
        ...
        println!("observe auth=enabled (loopback + token); wire requires Bearer");
    } else {
        println!("observe auth=disabled (loopback reads); wire disabled until token set");
    }
