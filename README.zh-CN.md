...
- 跑 Claude：确认本机有 Node.js 18+，在“连接”里选择 Claude。可使用 CC-Switch 代理，也可以切到“直连”并填写 Anthropic API key；Base URL 留空时使用 `https://api.anthropic.com`。
- 跑 Codex：先安装 Codex CLI：`npm i -g @openai/codex`，并确保 Lilia 能检测到 `codex app-server`。Lilia 需要 Codex CLI `0.128.0+` 的 app-server 协议能力。
- 走 CC-Switch：启动本地 CC-Switch，确认代理 URL 默认或配置为 `http://127.0.0.1:15721`。Codex 通过 CC-Switch 时，上游 provider 必须支持 OpenAI Responses API 和当前 Codex 模型。
- 走直连：切换当前 backend 的连接模式为“直连”，填写对应 API key。Claude 使用 `ANTHROPIC_API_KEY` 语义，Codex 使用 `OPENAI_API_KEY` 语义；密钥保存在系统密钥存储中，不会在设置页回显。
- 失败修复：连接页会把 Node 缺失、Codex CLI 缺失、app-server 版本过低、CC-Switch 不可达、直连缺少 API key 等状态映射成可执行建议。修复后点击“重新检测”，再回到对话页发送第一条消息。

## 功能状态

以下按当前真实接入面记录。只有已经能作为用户功能使用的项目标记为完成；部分接入和未接入项目均保持未完成。最近核对时间：2026-06-12。
